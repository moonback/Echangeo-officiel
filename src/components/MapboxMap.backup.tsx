...
const calculateDistanceOptimized = (lat1: number, lng1: number, lat2: number, lng2: number): number => {
  const key = `${lat1.toFixed(6)},${lng1.toFixed(6)},${lat2.toFixed(6)},${lng2.toFixed(6)}`;
  
  if (distanceCache.has(key)) {
    // Stratégie LRU - déplacer vers la fin
...
const MapboxMap = React.forwardRef<mapboxgl.Map, MapboxMapProps>(({
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      map.on('load', () => {
        ...
        // Le marqueur utilisateur sera géré dans un useEffect séparé
  ...
  // Mise à jour optimisée des marqueurs basée sur le hash
  useEffect(() => {
    updateMarkers();
